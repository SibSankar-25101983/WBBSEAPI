...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminSchoolInboxController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminSchoolInboxController");
                status = -1;
            }
        ...
        public ActionResult Inbox(string x)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Inbox/View", "AdminSchoolInboxController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetSchoolInboxList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MInbox().getAdminSchoolInboxList(page, limit, sortBy, direction, ref total, searchString);
        ...
        public ActionResult Inbox(HttpPostedFileBase[] postedFiles, VMInbox data)
        {
            try
            {
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Inbox", "AdminSchoolInbox", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MInbox().saveAdminSchoolInbox(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Inbox/Save", "AdminSchoolInboxController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetSchoolInboxView(string i)
        {
            ...
            try
            {
                View = new MInbox().getAdminSchoolInboxView(i);
            }
            catch (Exception ex)
            {
                MCommon.saveExceptionLog(ex.Message, "GetSchoolInboxView", "AdminSchoolInboxController");
            }
        ...
        public JsonResult GetSchoolInboxEdit(string i)
        {
            ...
            try
            {
                SchoolList = new MInbox().getAdminSchoolInboxEdit(i);
            }
            catch (Exception ex)
            {
                MCommon.saveExceptionLog(ex.Message, "GetSchoolInboxEdit", "AdminSchoolInboxController");
            }
