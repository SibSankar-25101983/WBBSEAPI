...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminPreSchoolInboxController : Controller
    {
        private int makeData(string encRoleId)
        {
                ...
                else
                {
                    ...
                    ******************************************************/
                    ...
                    string[,] columns = new string[,] {
                        ...
                        {"BodyText", "Notice", "false", "500"},
                        {"SchoolCount", "School(s)", "false", "100"},
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminPreSchoolInboxController");
                status = -1;
            }
        ...
        public ActionResult Inbox(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Inbox/View", "AdminPreSchoolInboxController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetSchoolInboxList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MInbox().getAdminPreSchoolInboxList(page, limit, sortBy, direction, ref total, searchString);
        ...
        public JsonResult GetLinkTypeList()
        {
            ...
            try
            {
                if (records.Count > 0)
                {
                    records.RemoveAll(x => x.LinkTypeId != GblFunctions.Base64Encode(LinkTypes.PDF) && x.LinkTypeId != GblFunctions.Base64Encode(LinkTypes.URL));
                }
            }
            catch
            {
        ...
        {
            var records = new MMstPreSchool().getMstPreSchoolListAutoComplete(Sanitizer.GetSafeHtmlFragment(s));

            return Json(records, JsonRequestBehavior.AllowGet);
        }

        [HttpPost, ValidateAntiForgeryToken]
        public ActionResult Inbox(HttpPostedFileBase[] postedFiles, VMInbox data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Inbox", "AdminPreSchoolInbox", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MInbox().saveAdminPreSchoolInbox(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Inbox/Save", "AdminPreSchoolInboxController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetSchoolInboxView(string i)
        {
            string View = string.Empty;

            try
            {
                View = new MInbox().getAdminPreSchoolInboxView(i);
            }
            catch (Exception ex)
            {
                MCommon.saveExceptionLog(ex.Message, "GetSchoolInboxView", "AdminPreSchoolInboxController");
            }
        ...
        [HttpGet]
        public JsonResult GetSchoolInboxEdit(string i)
        {
            string SchoolList = string.Empty;

            try
            {
                SchoolList = new MInbox().getAdminPreSchoolInboxEdit(i);
            }
            catch (Exception ex)
            {
                MCommon.saveExceptionLog(ex.Message, "GetSchoolInboxEdit", "AdminPreSchoolInboxController");
            }

            return Json(new { SchoolList }, JsonRequestBehavior.AllowGet);
        }
    }
}
