...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminRTIController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminRTIController");
                status = -1;
            }
        ...
        public ActionResult RTI(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "RTI/View", "AdminRTIController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        {
            var records = new MRTI().getRTIListDropDown();
        ...
        public JsonResult GetRTIList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MRTI().getRTIList(page, limit, sortBy, direction, ((searchString == null) ? string.Empty : GblFunctions.Base64Decode(searchString)), ref total, string.Empty);
        ...
        public ActionResult RTI(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("RTI", "AdminRTI", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MRTI().saveRTI(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "RTI/Save", "AdminRTIController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
