...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminRequisitionSlipController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminRequisitionSlipController");
                status = -1;
            }
        ...
        public ActionResult RequisitionSlip(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "RequisitionSlip/View", "AdminRequisitionSlipController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetRequisitionSlipList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MRequisitionSlip().getRequisitionSlipList(page, limit, sortBy, direction, MenuCode.RequisitionSlip, ref total, searchString);
        ...
        public ActionResult RequisitionSlip(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("RequisitionSlip", "AdminRequisitionSlip", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MRequisitionSlip().saveRequisitionSlip(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "RequisitionSlip/Save", "AdminRequisitionSlipController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
