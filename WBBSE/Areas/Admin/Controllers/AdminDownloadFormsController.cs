...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminDownloadFormsController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminDownloadFormsController");
                status = -1;
            }
        ...
        public ActionResult Forms(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Forms/View", "AdminCircularController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetDownloadFormsList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MDownloadForms().getDownloadFormsList(page, limit, sortBy, direction, MenuCode.DownloadForms, ref total, searchString);
        ...
        public ActionResult Forms(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Forms", "AdminDownloadForms", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MDownloadForms().saveDownloadForms(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Forms/Save", "AdminCircularController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
