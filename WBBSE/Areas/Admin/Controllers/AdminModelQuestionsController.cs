...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminModelQuestionsController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminModelQuestionsController");
                status = -1;
            }
        ...
        public ActionResult ModelQuestions(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "ModelQuestions/View", "AdminModelQuestionsController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetModelQuestionsList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MModelQuestions().getModelQuestionsList(page, limit, sortBy, direction, MenuCode.ModelQuestions, ref total, searchString);
        ...
        public ActionResult ModelQuestions(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("ModelQuestions", "AdminModelQuestions", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MModelQuestions().saveModelQuestions(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "ModelQuestions/Save", "AdminModelQuestionsController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
