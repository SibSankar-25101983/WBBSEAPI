...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminEBooksController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminEBooksController");
                status = -1;
            }
        ...
        public ActionResult EBooks(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "EBooks/View", "AdminEBooksController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetEBooksList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MEBooks().getEBooksList(page, limit, sortBy, direction, MenuCode.EBooks, ref total, searchString);
        ...
        public ActionResult EBooks(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("EBooks", "AdminEBooks", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MEBooks().saveEBook(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "EBooks/Save", "AdminEBooksController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
