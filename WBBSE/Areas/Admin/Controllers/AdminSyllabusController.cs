...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminSyllabusController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminSyllabusController");
                status = -1;
            }
        ...
        public ActionResult Syllabus(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Syllabus/View", "AdminSyllabusController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetSyllabusList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MSyllabus().getSyllabusList(page, limit, sortBy, direction, MenuCode.Syllabus, ref total, searchString);
        ...
        public ActionResult Syllabus(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Syllabus", "AdminSyllabus", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MSyllabus().saveSyllabus(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Syllabus/Save", "AdminSyllabusController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
