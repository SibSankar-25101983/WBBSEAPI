...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminSyllabusCurriculumController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminSyllabusCurriculumController");
                status = -1;
            }
        ...
        public ActionResult SyllabusCurriculum(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "SyllabusCurriculum/View", "AdminSyllabusCurriculumController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetSyllabusCurriculumList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MSyllabusCurriculum().getSyllabusCurriculumList(page, limit, sortBy, direction, MenuCode.SyllabusCurriculum, ref total, searchString);
        ...
        public ActionResult SyllabusCurriculum(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("SyllabusCurriculum", "AdminSyllabusCurriculum", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MSyllabusCurriculum().saveSyllabusCurriculum(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "SyllabusCurriculum/Save", "AdminSyllabusCurriculumController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
