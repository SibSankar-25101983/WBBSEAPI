...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminContentController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminContentController");
                status = -1;
            }
        ...
        public ActionResult Contents(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Content/View", "AdminContentController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        {
            var records = new MContent().getContentEditListDropDown();
            return Json(records, JsonRequestBehavior.AllowGet);
        }

        [HttpGet]
        public JsonResult GetContentList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MContent().getContentList(page, limit, sortBy, direction, ((searchString == null) ? string.Empty : GblFunctions.Base64Decode(searchString)), ref total, string.Empty);
        ...
        [ValidateInput(false)]
        public ActionResult Contents(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Contents", "AdminContent", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MContent().saveContent(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Circular/Save", "AdminCircularController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
