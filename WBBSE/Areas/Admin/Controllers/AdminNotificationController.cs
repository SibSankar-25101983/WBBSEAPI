...
namespace WBBSE.Areas.Admin.Controllers
{
    public class AdminNotificationController : Controller
    {
                ...
                else
                {
                    ...
                    ******************************************************/
                    ...
                    string[,] columns = new string[,] {
                        ...
                        {"NotificationType", "Type", "false", "80"},
        ...
        public ActionResult Notification(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Notification/View", "AdminNotificationController");
                return Redirect("~/Error/Unexpected.html");
            }
        }

        [HttpGet]
        public JsonResult GetNotificationList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            int total = 0;

            var records = new MNotification().getNotificationList(page, limit, sortBy, direction, MenuCode.Notification, ref total, searchString);
        ...
        {
            var records = new MNotification().getNotificationTypeListDropDown();

            return Json(records, JsonRequestBehavior.AllowGet);
        }

        [HttpPost, ValidateAntiForgeryToken]
        public ActionResult Notification(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Notification", "AdminNotification", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MNotification().saveNotification(postedFiles, data, ref errDesc);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Notification/Save", "AdminCircularController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
    }
