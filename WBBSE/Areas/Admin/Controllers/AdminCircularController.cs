...
namespace WBBSE.Areas.Admin.Controllers
{
    ...
    public class AdminCircularController : Controller
    {
        private int makeData(string encRoleId)
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "makeData", "AdminCircularController");
                status = -1;
            }
        ...
        public ActionResult Circular(string x)
        {
            try
            {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "Circular/View", "AdminCircularController");
                return Redirect("~/Error/Unexpected.html");
            }
        }
        ...
        public JsonResult GetCircularList(int? page, int? limit, string sortBy, string direction, string searchString = null)
        {
            ...
            var records = new MCircular().getCircularList(page, limit, sortBy, direction, MenuCode.Circulars, ref total, searchString);
        ...
        public ActionResult Circular(HttpPostedFileBase[] postedFiles, VMContentDetails data)
        {
            try
            {
                int status = 0, err = 0;
                ...
                if (mode == Mode.ERROR)
                {
                    ...
                    return RedirectToAction("Circular", "AdminCircular", rvd);
                }

                if (mode == Mode.DELETE) //if mode = delete, bypass model state checking
                {
                    err = new MCircular().saveCircular(postedFiles, data, ref errDesc);
