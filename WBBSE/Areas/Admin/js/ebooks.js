...
function GetLinkTypeList(d) {
    ...
    try {
        $.ajax({
            url: "/AdminDownloadForms/GetLinkTypeList",
            type: "GET",
            dataType: "json",
...
function Add() {
    ...
    $("#lblModalHeader").text("E-Books :: Add New");
            ...
            if ($("#ddlLinkType option:selected").text().toUpperCase() == Content.LinkType.PDF) {
                if ($("#e").val() == setET(1)) {
                    ...
                    if (postedFiles[0].files[0].size > Content.EBookPdfAllowedSize) {
                        $('#AlertInfo').show();
                        $('#AlertInfoMsg').text(Content.Validation.EBookPdfFileSize);
                        $('#popupModal').scrollTop(0);
                        return false;
                    }
                }
...
function Edit(e) {
    ...
    try {
        ...
        $("#lblModalHeader").text("E-Books :: Edit");
