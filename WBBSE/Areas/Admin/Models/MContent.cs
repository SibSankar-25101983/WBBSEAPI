...
namespace WBBSE.Areas.Admin.Models
{
    ...
    {
        public List<VMContentDetails> getContentEditListDropDown()
        {
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "GetModuleList/getContentEditListDropDown(MContent)", "AdminContentController");
            }
            finally
            {
                dt = null;
        ...
        public List<VMContentDetails> getContentList(int? page, int? limit, string sortBy, string direction, string menuCode, ref int total, string searchString, string archiveYN = "A")
        {
            DataTable dt = new DataTable();
            List<VMContentDetails> result = new List<VMContentDetails>();
            ...
            try
            {
                dt = new Content().GetContentList(searchString, menuCode, page, limit, ref total, archiveYN);
            ...
            {
                MCommon.saveExceptionLog(ex.Message, "GetContentList/getContentList(MContent)", "AdminContentController");
            }
            finally
            {
                dt = null;
        ...
        public int saveContent(HttpPostedFileBase[] postedFiles, VMContentDetails data, ref string errDesc)
        {
            ...
            string bodyText = string.Empty, URL = string.Empty, newYN = string.Empty, archiveYN = string.Empty, menuCode = string.Empty, menuName = string.Empty;
            ...
            try
            {
                ...
                menuCode = GblFunctions.Base64Decode(Sanitizer.GetSafeHtmlFragment(data.MenuCode.Trim()));
                menuName = (Sanitizer.GetSafeHtmlFragment(data.MenuName) ?? string.Empty).Trim();

                if (mode != Mode.DELETE)
                {
                    linkTypeId = Convert.ToInt32(GblFunctions.Base64Decode(Sanitizer.GetSafeHtmlFragment(data.LinkTypeId.Trim())));
                    if (linkTypeId != Convert.ToInt32(LinkTypes.CONTENT) && linkTypeId != Convert.ToInt32(LinkTypes.PDF) && linkTypeId != Convert.ToInt32(LinkTypes.URL))
                    {
                        err = 1;
                        errDesc = Message.OperationError;
                        return err;
                    }
                    bodyText = Sanitizer.GetSafeHtmlFragment((data.BodyTextOriginal ?? string.Empty).Trim()).Replace("script", "");
                    ...
                    if (menuCode == DefaultSetting.DefaultVal)
                    {
                        err = 1;
                        errDesc = Message.OperationError;
                        return err;
                    ...
                    if (linkTypeId.ToString() == LinkTypes.CONTENT && bodyText == string.Empty)
                    {
                        err = 2;
                    ...
                    //URL CHECKING UNSUCCESSFUL
                    //if (linkTypeId.ToString() == LinkTypes.URL)
                    //{
                    //    Regex reg = new Regex("/(http(s)?://.)?(www.)?[-a-zA-Z0-9@:%._+~#=]{2,256}.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&//=]*)/g");

                    //    if(!reg.IsMatch(URL))
                    //    {
                    //        err = 2;
                    //        errDesc = Message.Content.InvalidURL;
                    //        return err;
                    //    }
                    //}
                    ...
                    {
                        //file size checking
                        if (postedFiles[0].ContentLength > MaxFileSize.PDFEBook)
                        {
                            err = 2;
                            errDesc = Message.FileUpload.MaxFileSizeBookPdf;
                            return err;
                        }
                        ...
                        dbFilePath = FilePath.Content;
                    }
                }

                filePath = HttpContext.Current.Server.MapPath("~/ReadWriteData/Content/");
                ...
                err = new DAL.Common().SaveContentDetails(ContentId, menuCode, linkTypeId, menuName, bodyText, string.Empty
                                                        , URL, fileName, dbFilePath, mode, ipAddress, createdBy, ref errDesc, newYN, archiveYN, MenuType.MainMenu);
                ...
                {
                    MCommon.saveExceptionLog(errDesc, "Contents/saveContent(MContent)", "AdminContent");
                    err = 1;
                    errDesc = Message.OperationError;
                    return err;
                }
            ...
            {
                err = 1;
                MCommon.saveExceptionLog(ex.Message, "Contents/saveContent(MContent)", "AdminContent");
            }
