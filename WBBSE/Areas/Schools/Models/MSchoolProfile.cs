    ...
    public class MSchoolProfile
    {
        public int chkDuplicateContactMstSchool(string emailId, string phoneNo, string mobileNo, string schoolId, string entType, ref string errDesc, string diseCode, string indexNo, string subDivisionId)
        {
            ...
            try
            {
                ...
                {
                    err = new School().ChkDuplicateContactMstSchool(emailId, phoneNo, mobileNo, schoolId, entType, ref errDesc, diseCode, indexNo, Convert.ToInt32(GblFunctions.Base64Decode(Sanitizer.GetSafeHtmlFragment(subDivisionId))));
                }
                ...
                if (err == 1)
                {
                    MCommon.saveExceptionLog(errDesc, "SchoolProfile/chkDuplicateContactMstSchool(MSchoolProfile)", "SchoolProfileController");
                }
            }
            catch (Exception ex)
            {
                MCommon.saveExceptionLog(ex.Message, "SchoolProfile/chkDuplicateContactMstSchool(MSchoolProfile)", "SchoolProfileController");
            }

            return err;
        }

        public int updateSchoolProfile(VMSchoolProfile data, ref string errDesc)
        {
            int err = 0, createdBy = 0, userTypeid = 0;
            string mode = string.Empty, ipAddress = string.Empty, indexNo = string.Empty;
            ...
            try
            {
                ...
                if (mode != Mode.DELETE)
                {
                    ...
                    err = chkDuplicateContactMstSchool((Sanitizer.GetSafeHtmlFragment(data.EmailId) ?? "").Trim(), (Sanitizer.GetSafeHtmlFragment(data.PhoneNo) ?? "").Trim()
                                                        ...
                                                        , EntType.EDIT, ref errDesc, (Sanitizer.GetSafeHtmlFragment(data.DISECode) ?? "").Trim()
                                                        , string.Empty, DefaultSetting.DefaultVal);
                ...
                err = new School().UpdateSchoolProfile(data, userTypeid, mode, ipAddress, createdBy, ref errDesc, indexNo);
            }
            catch (Exception ex)
            {
                err = 1;
            ...
            if (err == 1)
            {
                MCommon.saveExceptionLog(errDesc, "SchoolProfile/updateSchoolProfile(MSchoolProfile)", "SchoolProfileController");
            }
        ...
        public string chkSchoolEditPermission(Int64 schoolId, string editFor)
        {
            ...
            {
                data = "N";
                MCommon.saveExceptionLog(ex.Message, "makeData/chkSchoolEditPermission(MSchoolProfile)", "SchoolProfileController");
            }
