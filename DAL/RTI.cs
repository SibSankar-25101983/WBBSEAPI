...
namespace DAL
{
    public class RTI
    {
        public DataTable GetRTIList(string searchString, string menuCode, int? pageNo, int? pageSize, ref int totalCount, string archiveYN)
        {
        ...
        public DataTable GetRTIListDropDown()
        {
            SqlCommand oCmd = new SqlCommand();
            DataTable dt = new DataTable();
            ...
            {
                oCmd.CommandType = CommandType.StoredProcedure;
                oCmd.CommandText = "GetRTIListDropDown";
                DBUtility.ExecuteForSelect(oCmd, dt);
            }
            catch
            {
